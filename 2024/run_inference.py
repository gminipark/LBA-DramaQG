...
from dataset import VideoTextGenerationDataset
...
def get_model(args):
    ...
    cache_dir = (
        args.cache_dir
        if args.cache_dir
        else os.path.join("./", args.model_name_or_path.split("/")[-1])
    )

    model = LlavaNextVideoForConditionalGeneration.from_pretrained(
        args.model_name_or_path, torch_dtype=torch.float16, cache_dir=cache_dir
    )
...
def get_processor(args):
    ...
    processor_name_or_path = (
        args.processer_name_or_path
        if args.processer_name_or_path
...
def get_decoding_strategy(strategy_name):
    ...
    if strategy_name == "greedy":
        ...
        parameters = {"max_new_tokens": 50, "num_beams": 4}

    elif strategy_name == "constrastive":
        parameters = {"max_new_tokens": 40, "penalty_alpha": 0.6, "top_k": 4}
        ...
        parameters = {"max_new_tokens": 50, "do_sample": True, "top_p": 0.95}
    return parameters
...
def add_args(parser):
    ...
    parser.add_argument(
        "--decoding_strategy",
        default="beam",
        choices=["greedy", "beam", "constrastive", "diverse", "sample"],
    )
    parser.add_argument(
        "--input_path",
        type=str,
        default="../../Integration-Outputs/output_KAIST.json",
        required=True,
    )
    parser.add_argument(
        "--output_path", type=str, default="../../Integration-Outputs/output_KHU.json"
    )
...
def main():
    ...
    test_dataset = VideoTextGenerationDataset(args, examples, processor)
    test_dataloader = DataLoader(
        test_dataset, shuffle=False, batch_size=1, collate_fn=test_dataset.collate_fn
    )
    ...
    for i, batch in enumerate(tqdm(test_dataloader)):
        ...
        generated_outputs = model.generate(**inputs, **parameters)

        outputs += generated_outputs[:, inputs["input_ids"][0].shape[-1] :]
